...
const SignUpProps = ({ GoogleIcon = GoogleIconImg, Title }) => {
  ...
  const handleCompanyNameInput = (e) => {
    ...
    if (!e.target.value) {
      setErrors((prev) => ({ ...prev, lastName: "Company Name is required" }));
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://wastewave-backend.onrender.com/api/company/register",
        {
      ...
      if (status !== "PENDING") {
        ...
        navigate("/company/verification", { email, userId: data.userId });
      }
    } catch (error) {
